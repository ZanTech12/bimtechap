...
// ✅ Extract the real ID; the query can never fire with a class name
...
const TestResults = () => {
  ...
  const { data: sessions } = useQuery({ queryKey: ['sessions'], queryFn: () => sessionsAPI.getAll() });
    ...
    // ✅ Auto-select: prefer the active term, otherwise fall back sensibly
  useEffect(() => {
    if (showPushModal && terms?.data?.length) {
      const active = terms.data.find(t => t.status === 'active')
                  ?? terms.data.find(t => t.isActive === true)          // fallback 1: isActive flag
                  ?? terms.data[terms.data.length - 1];                  // fallback 2: latest created
      if (active) {
        setPushTermId(idStr(active));
        setPushSessionId(idStr(active.sessionId ?? active.session));
      }
    }
  ...
  // ✅ Class filtering with normalized IDs
  const availableClasses = useMemo(() => {
    if (!classes?.data) return [];
    if (!isTeacher) return classes.data;
    if (!myAssignments?.data) return [];
    const ids = new Set(myAssignments.data.map(a => idStr(a.class_id ?? a.classId)).filter(Boolean));
    return classes.data.filter(c => ids.has(idStr(c)));
  }, [isTeacher, classes?.data, myAssignments?.data]);

  // ✅ Subject filtering with normalized IDs
  const availableSubjects = useMemo(() => {
    if (!subjects?.data) return [];
    if (isTeacher) {
      if (!selectedClass || !myAssignments?.data) return [];
      const cls = idStr(selectedClass);
      return subjects.data.filter(sub => myAssignments.data.some(a =>
        idStr(a.class_id ?? a.classId) === cls && idStr(a.subject_id ?? a.subjectId) === idStr(sub)
      ));
    }
  ...
  const availableTests = useMemo(() => {
    ...
    let f = tests.data;
    if (selectedClass) f = f.filter(t => idStr(t.classId) === idStr(selectedClass));
    if (selectedSubject) f = f.filter(t => idStr(t.subjectId) === idStr(selectedSubject));
    return f;
  ...
  // ✅ PATCH 2: string-vs-string comparison (was number === string → always false)
  const handleExport = () => {
    if (selectedTest) {
      const t = availableTests.find(t => idStr(t) === selectedTest);
      downloadCSV(selectedTest, `${t?.title || 'test'}_results.csv`);
    }
  };

  const handlePrint = () => window.print();

  // ✅ PATCH 2: string-vs-string comparison
  const getSelectedTestDetails = () => {
    if (!selectedTest) return null;
    return availableTests.find(t => idStr(t) === selectedTest) || null;
  };
  ...
  const processedResults = useMemo(() => {
    ...
    return [...results.data.results].filter(r => !r.notTaken)
      ...
      .sort((a, b) => b.percentage - a.percentage || String(a.studentName || '').localeCompare(String(b.studentName || '')))
      .map((r, i) => ({ ...r, rank: i + 1 }));
  ...
  // ✅ PATCH 3: send clean parsed Ints for classId/subjectId/termId/sessionId
  const handlePushResults = async () => {
    ...
    const cId = idStr(tObj?.classId);
    const sId = idStr(tObj?.subjectId);
    ...
    try {
      const response = await teacherCAAPI.pushTestResultsAsCA(selectedTest, {
        ...
        termId: parseInt(pushTermId),
        sessionId: parseInt(pushSessionId),
        assessments, pushType, overwriteTestScore: pushOverwriteMode
      });
      if (response.success) {
  ...
  const filteredResults = useMemo(() => {
    ...
    return data.sort((a, b) => {
      let va, vb;
      switch (sortBy) {
        case 'name': va = String(a.studentName || ''); vb = String(b.studentName || ''); return sortDir === 'asc' ? va.localeCompare(vb) : vb.localeCompare(va);
  ...
  const notTakenStudents = useMemo(() => {
    if (!results?.data?.results) return [];
    return results.data.results.filter(r => r.notTaken).sort((a, b) => String(a.studentName || '').localeCompare(String(b.studentName || '')));
  }, [results?.data?.results]);
  ...
  const selTest = getSelectedTestDetails();
  ...
  return (
    <div className={`tr-root ${darkMode ? 'tr-dark-mode' : ''}`}>
        ...
        <div className="tr-header-inner">
          <div className="tr-header-left">
            ...
            <div>
              ...
              <p className="tr-header-sub">{isTeacher ? 'View and push results for your assigned subjects' : 'Analyze and manage all test results'}</p>
            </div>
          </div>
          <div className="tr-header-actions">
            <button className="tr-dark-toggle" onClick={() => setDarkMode(d => !d)} title="Toggle dark mode">{darkMode ? '☀️' : '🌙'}</button>
            <button className="tr-btn tr-btn-ghost" onClick={handleExport} disabled={!selectedTest}>
              <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"/><polyline points="7 10 12 15 17 10"/><line x1="12" y1="15" x2="12" y2="3"/></svg>
              Export CSV
            </button>
            <button className="tr-btn tr-btn-ghost" onClick={handlePrint} disabled={!selectedTest}>
            ...
            <button className="tr-btn tr-btn-primary" onClick={() => handleOpenPushModal('test')} disabled={processedResults.length === 0}>
              <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><line x1="12" y1="5" x2="12" y2="19"/><line x1="5" y1="12" x2="19" y2="12"/></svg>
              Push as CA
            </button>
            <button className="tr-btn tr-btn-warning" onClick={() => handleOpenPushModal('exam')} disabled={processedResults.length === 0}>
              <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><line x1="12" y1="5" x2="12" y2="19"/><line x1="5" y1="12" x2="19" y2="12"/></svg>
              Push as Exam
            </button>
          </div>
      ...
      {selectedTest && stats && (
        <div className="tr-stats">
          <div className="tr-stat tr-stat--total">
            ...
            <div><div className="tr-stat-val">{stats.totalStudents ?? processedResults.length}</div><div className="tr-stat-label">Students</div></div>
          </div>
          <div className="tr-stat tr-stat--taken">
            <div className="tr-stat-icon" style={{ background: '#ecfdf5', color: '#059669' }}>
              <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><polyline points="20 6 9 17 4 12"/></svg>
            </div>
            <div><div className="tr-stat-val">{stats.taken ?? processedResults.length}</div><div className="tr-stat-label">Taken</div></div>
          </div>
          <div className="tr-stat tr-stat--avg">
            ...
            <div><div className="tr-stat-val">{stats.averageScore || 0}%</div><div className="tr-stat-label">Average</div></div>
          </div>
          <div className="tr-stat tr-stat--pass">
            <div className="tr-stat-icon" style={{ background: '#ecfdf5', color: '#10b981' }}>
              <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M22 11.08V12a10 10 0 1 1-5.93-9.14"/><polyline points="22 4 12 14.01 9 11.01"/></svg>
            </div>
            <div><div className="tr-stat-val">{stats.passRate || 0}%</div><div className="tr-stat-label">Pass Rate</div></div>
          </div>
          <div className="tr-stat tr-stat--high">
            <div className="tr-stat-icon" style={{ background: '#fffbeb', color: '#d97706' }}>
            ...
            <div><div className="tr-stat-val">{stats.highestScore || 0}%</div><div className="tr-stat-label">Highest</div></div>
          </div>
          <div className="tr-stat tr-stat--low">
            ...
            <div><div className="tr-stat-val">{stats.lowestScore || 0}%</div><div className="tr-stat-label">Lowest</div></div>
          </div>
        </div>
      )}
      ...
      <div className="tr-panel">
        ...
        <div className="tr-filters-row">
          <div className="tr-filter-group">
            ...
            <div className="tr-select-wrap">
              <select className="tr-filter-select" value={selectedClass} onChange={e => setSelectedClass(e.target.value)}>
                ...
                {availableClasses.map(c => { const cid = idStr(c);
          ...
          <div className="tr-filter-group">
            ...
            <div className="tr-select-wrap">
              <select className="tr-filter-select" value={selectedSubject} onChange={e => setSelectedSubject(e.target.value)} disabled={!selectedClass}>
                <option value="">{selectedClass ? 'Select Subject' : '— Select class first —'}</option>
                {availableSubjects.map(s => { const sid = idStr(s);
          ...
          <div className="tr-filter-group">
            ...
            <div className="tr-select-wrap">
              <select className="tr-filter-select" value={selectedTest} onChange={e => setSelectedTest(e.target.value)} disabled={!selectedSubject}>
                <option value="">{selectedSubject ? 'Select Test' : '— Select subject first —'}</option>
                {availableTests.map(t => { const tid = idStr(t);
                  return <option key={tid} value={tid}>{t.title}</option>; })}
              </select>
              <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><polyline points="6 9 12 15 18 9"/></svg>
            </div>
            {selectedSubject && availableTests.length === 0 && <span className="tr-filter-hint">No tests found for this class & subject.</span>}
          </div>
          <div className="tr-filter-actions">
            <div className="tr-search-wrap" style={{ minWidth: 200 }}>
              <svg className="tr-search-icon" width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><circle cx="11" cy="11" r="8"/><line x1="21" y1="21" x2="16.65" y2="16.65"/></svg>
              <input type="text" className="tr-search-input" placeholder="Search student..." value={searchQuery} onChange={e => setSearchQuery(e.target.value)} />
      ...
      {processedResults.length > 0 && (
        <div className="tr-dist">
          ...
          <div className="tr-dist-body">
            {scoreDistribution.map(rng => (
              <div className="tr-dist-row" key={rng.label}>
                <span className="tr-dist-label">{rng.label}</span>
                <div className="tr-dist-bar">
                  <div className="tr-dist-fill" style={{ width: `${Math.max((rng.count / maxDistCount) * 100, rng.count > 0 ? 12 : 0)}%`, background: rng.gradient }}>
                    {rng.count > 0 && <span>{rng.count}</span>}
                  </div>
                </div>
              </div>
            ))}
        ...
        <div className="tr-insights">
          <div className="tr-insights-toggle" onClick={() => setShowPerfInsights(v => !v)}>
            <div className="tr-insights-toggle-left">
              ...
              <h3>Performance Insights <span className="tr-insights-badge">{performanceInsights.length}</span></h3>
            </div>
          ...
          {showPerfInsights && (
            <div className="tr-insights-body">
              {performanceInsights.map((ins, i) => (
                <div className={`tr-insight tr-insight--${ins.type}`} key={i}>
                  <span className="tr-insight-icon">{ins.icon}</span>
                  <span>{ins.text}</span>
                </div>
              ))}
            </div>
        ...
        <div className="tr-grade-row">
          {gradeDistribution.map(([grade, count]) => (
            <span className="tr-grade-pill" key={grade}>
              <span className="tr-grade-pill-grade">{grade}</span>
      ...
      {/* ===== RESULTS ===== */}
      <div className="tr-results-card tr-table-section">
        <div className="tr-results-bar">
          <div className="tr-results-left">
            <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/><polyline points="14 2 14 8 20 8"/></svg>
            <h3>{selTest ? selTest.title : 'Results'}</h3>
          </div>
          <div className="tr-results-right">
            <span className="tr-results-count">{filteredResults.length} results</span>
            {searchQuery && <span className="tr-results-search-count">filtered from {processedResults.length}</span>}
          </div>
        </div>
        ...
        {filteredResults.length === 0 ? (
          <div className="tr-empty">
            <div className="tr-empty-icon">
              <svg width="34" height="34" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/><polyline points="14 2 14 8 20 8"/></svg>
            </div>
            <h3>{!selectedTest ? 'No test selected' : (resultsLoading ? 'Loading results...' : 'No results yet')}</h3>
            <p>{!selectedTest ? 'Select a class, subject, and test above to view results.' : 'No student has taken this test yet.'}</p>
          </div>
        ) : (
          <div className="tr-table-wrap">
            <table className="tr-table">
              <thead>
                <tr>
                  <th className="sortable" onClick={() => handleSort('rank')}>Rank {getSortIcon('rank')}</th>
                  ...
                  <th>Grade</th>
                  <th className="sortable" onClick={() => handleSort('ca')}>CA (20) {getSortIcon('ca')}</th>
                  <th className="sortable" onClick={() => handleSort('date')}>Date Taken {getSortIcon('date')}</th>
                </tr>
              </thead>
              <tbody>
                {filteredResults.map(r => {
                  ...
                  const rankCls = r.rank === 1 ? 'tr-rank--gold' : r.rank === 2 ? 'tr-rank--silver' : r.rank === 3 ? 'tr-rank--bronze' : '';
                  return (
                    <tr key={r.studentId}
                        className={hoveredStudent === r.studentId ? 'tr-highlighted' : ''}
                        onMouseEnter={() => setHoveredStudent(r.studentId)}
                        onMouseLeave={() => setHoveredStudent(null)}>
                      <td><span className={`tr-rank ${rankCls}`}>{r.rank}</span></td>
                      <td>
                        <div className="tr-student">
                          ...
                          <div>
                            <div className="tr-student-name">{r.studentName}</div>
                            <div className="tr-student-adm">{r.admissionNumber}</div>
                          </div>
                        </div>
                      </td>
                      <td>
                        <div className="tr-score-cell">
                          <div className="tr-ring">
                            <svg width="42" height="42">
                              <circle cx="21" cy="21" r="18" fill="none" stroke="#e2e8f0" strokeWidth="4"/>
                              <circle cx="21" cy="21" r="18" fill="none" stroke={color.bar} strokeWidth="4" strokeDasharray={`${(r.percentage / 100) * 113} 113`} strokeLinecap="round"/>
                            </svg>
                            <span className="ring-text" style={{ color: color.text }}>{r.percentage}%</span>
                          </div>
                          <div>
                            <div className="tr-score-frac">{r.score}/{r.totalQuestions}</div>
                            <div className="tr-score-status" style={{ color: r.passed ? '#059669' : '#dc2626' }}>{r.passed ? '✓ Passed' : '✗ Failed'}</div>
                          </div>
                        </div>
                      </td>
                      <td>
                        <span className="tr-grade" style={{ background: color.bg, color: color.text }}>{grade.grade} {grade.emoji}</span>
                      </td>
                      <td><span className="tr-ca">{r.caScore}/20</span></td>
      ...
      <div className="tr-cards">
        {filteredResults.length === 0 ? (
          <div className="tr-empty">
            <h3>{!selectedTest ? 'No test selected' : 'No results yet'}</h3>
            <p>{!selectedTest ? 'Select a class, subject, and test to view results.' : 'No student has taken this test yet.'}</p>
          </div>
        ) : filteredResults.map(r => {
          const color = getScoreColor(r.percentage);
          const grade = getGrade(r.percentage);
              ...
              <div className="tr-card-top">
                ...
                <div className="tr-card-ring">
                  <svg viewBox="0 0 50 50">
                    ...
                    <circle cx="25" cy="25" r="21" fill="none" stroke={color.bar} strokeWidth="5" strokeDasharray={`${(r.percentage / 100) * 132} 132`} strokeLinecap="round"/>
                  </svg>
                  <span className="ring-text" style={{ color: color.text }}>{r.percentage}%</span>
                </div>
              </div>
              <div className="tr-card-mid">
                <span className="tr-card-frac">{r.score}/{r.totalQuestions}</span>
                <span className="tr-card-pass" style={{ background: color.bg, color: color.text }}>{r.passed ? '✓ Passed' : '✗ Failed'}</span>
                <span className="tr-grade" style={{ background: color.bg, color: color.text }}>{grade.grade} {grade.emoji}</span>
              </div>
              <div className="tr-card-bottom">
                <span className="tr-ca">{r.caScore}/20</span>
                <span className="tr-card-date">{r.dateTaken ? new Date(r.dateTaken).toLocaleDateString() : '—'}</span>
              </div>
            </div>
          );
        })}
        ...
        <div className="tr-notaken">
          <div className="tr-notaken-bar">
            <h3>
              ...
              Students Who Haven't Taken This Test
              <span className="tr-notaken-badge">{notTakenStudents.length}</span>
            </h3>
          </div>
          <div className="tr-notaken-list">
            {notTakenStudents.map(r => (
              <span className="tr-notaken-chip" key={r.studentId}>{r.studentName}</span>
            ))}
          </div>
        </div>
      )}
      ...
      {/* ===== PUSH MODAL ===== */}
      {showPushModal && (
        <div className="tr-modal-overlay" onClick={handleClosePushModal}>
          <div className="tr-modal" onClick={e => e.stopPropagation()}>
            <div className="tr-modal-handle" />
            <div className="tr-modal-header">
              <div className="tr-modal-left">
                <div className={`tr-modal-icon ${pushType === 'test' ? 'tr-modal-icon--test' : 'tr-modal-icon--exam'}`}>
                  {pushType === 'test'
                ...
                <div>
                  <h3 className="tr-modal-title">{pushType === 'test' ? 'Push Test Scores as CA' : 'Push Scores as Exam'}</h3>
                  <p className="tr-modal-sub">{pushType === 'test' ? 'Scores will be written into the Continuous Assessment test column (out of 20).' : 'Scores will be added to the exam column (out of 20).'}</p>
                </div>
              </div>
              <button className="tr-modal-close" onClick={handleClosePushModal}>×</button>
            </div>

            <div className="tr-modal-body">
              <span className={`tr-push-badge ${pushType === 'test' ? 'tr-push-badge--test' : 'tr-push-badge--exam'}`}>
                {pushType === 'test' ? '📝 CA Test Scores' : '📄 Exam Scores'}
              </span>

              {selTest && (
                <div className={`tr-info ${pushType === 'test' ? 'tr-info--test' : 'tr-info--exam'}`}>
                  <div className="tr-info-title">
                    {pushType === 'test' ? '📋 Test Information' : '📋 Exam Information'}
                  </div>
                  <div className="tr-info-item"><strong>Test:</strong> {selTest.title}</div>
                  <div className="tr-info-item"><strong>Class:</strong> {selTest.classId?.name || idStr(selTest.classId)}</div>
                  <div className="tr-info-item"><strong>Subject:</strong> {selTest.subjectId?.name || idStr(selTest.subjectId)}</div>
                  <div className="tr-info-item"><strong>Students:</strong> {processedResults.length}</div>
                </div>
              )}
              ...
              {pushType === 'test' && (
                <div className="tr-toggle-row">
                  ...
                  <span><strong>Overwrite mode:</strong> replace existing test scores instead of only updating blank ones.</span>
                </div>
              )}

              <div className="tr-preview-head">
                <span className="tr-preview-title">Score Preview (out of 20) {hasEditedScores && <span className="tr-edited-dot" />}</span>
                <button type="button" className="tr-preview-reset" onClick={handleResetPushScores}>Reset to auto</button>
              </div>
              <div className="tr-preview">
                <table>
                  <thead>
                    <tr><th>Student</th><th>%</th><th>Score /20</th></tr>
                  </thead>
                  <tbody>
                    {processedResults.slice(0, 30).map(r => (
                      <tr key={r.studentId}>
                        <td>{r.studentName}</td>
                        <td>{r.percentage}%</td>
                        <td>
                          <input
                            type="number"
                            className={`tr-preview-input ${pushType === 'test' ? 'tr-preview-input--test' : 'tr-preview-input--exam'}`}
                            min="0" max="20" step="0.5"
                            value={pushScores[r.studentId] ?? convertScoreTo20(r.percentage)}
                ...
                {processedResults.length > 30 && <div className="tr-preview-more">+ {processedResults.length - 30} more students...</div>}
              </div>

              {pushConfirm && (
                <div className="tr-confirm" style={{ marginTop: 12 }}>
                  ...
                  Ready to push {processedResults.length} scores. This will modify CA records — click <strong>Confirm &amp; Push</strong> to proceed.
                </div>
              )}
            </div>

            <div className="tr-modal-footer">
              <button type="button" className="tr-btn tr-btn-cancel" onClick={handleClosePushModal}>Cancel</button>
              <button
                type="button"
                className={`tr-btn ${pushConfirm ? 'tr-btn-confirm' : 'tr-btn-primary'}`}
                disabled={pushing || processedResults.length === 0}
                onClick={() => { if (!pushConfirm) { setPushConfirm(true); } else { handlePushResults(); } }}>
                {pushing
                  ? (<><span className="tr-btn-spinner" /> Pushing...</>)
                  : pushConfirm
                    ? '⚠️ Confirm & Push'
                    : `Push ${processedResults.length} Scores`}
              </button>
            </div>
          </div>
        </div>
