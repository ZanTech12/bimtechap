...
const Sidebar = ({ items, collapsed, isOpen }) => {
  ...
  // ✅ LIVE site info from the shared hook (cached — same fetch as report-card pages).
  // The backend returns ABSOLUTE media URLs, so logoUrl is ready to use as-is.
  const site = useSchoolSettings();

  // Local fallback state while loading or if nothing is set yet
      ...
      <div className="sidebar-content">
        ...
        <div className="sidebar-logo">
          <div className="logo-mark">
            {logoUrl ? (
              <img
                // ✅ Backend returns the full absolute URL — no prefix needed
                src={logoUrl}
                alt={`${shortName} Logo`}
              ...
              <div className="logo-placeholder">
                {/* Fallback if no logo is uploaded yet */}
