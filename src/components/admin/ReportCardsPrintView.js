...
import { reportCardsAPI, termsAPI, principalCommentsAPI, studentsAPI } from '../../api';
...
const buildStudentPhotoUrl = (student) => {
  ...
  // 4. Final fallback: construct from student ID
...
const ReportCardsPrintView = () => {
  ...
  // ============================================
  // ✅ FETCH ALL STUDENTS (won't crash if studentsAPI.getAll missing)
  // ============================================
  ...
  // ============================================
  // ✅ PHOTO LOOKUP MAP — tolerant of id/._id and response shapes
  // ============================================
  ...
  // ============================================
  // ✅ PRINCIPAL COMMENTS MAP — tolerant id extraction (id / _id / object)
  // ============================================
    ...
    comments.forEach((comment) => {
      const rawStudentId =
        ...
        (typeof comment.student === 'object'
          ? docId(comment.student)
          : comment.student);

      if (rawStudentId != null) {
        map[rawStudentId.toString()] =
          comment.comment || comment.principalComment || comment.text || '';
      }
    });
  ...
  const getPrincipalComment = (studentId) => {
    ...
    return principalCommentsMap[studentId.toString()] || '';
  };
  ...
  const compactA4Styles = `
    ...
    .a4-document .header-logo { width: 84px !important; height: 84px !important; object-fit: contain !important; }
  ...
  // ✅ Missing URL params state (was silently showing "No Data Available")
  if (!termId || !classIdsString) {
    return (
      <div className="print-error-page">
        <h3>Missing Print Parameters</h3>
        <p>This page requires <code>termId</code> and <code>classIds</code> in the URL.</p>
        <p style={{ fontSize: 12, color: '#888' }}>
          Got: termId={termId || '—'} · classIds={classIdsString || '—'}
        </p>
  ...
  // ✅ Safe destructure — never crashes on missing shapes
  const term = printData?.term || {};
  const session = printData?.session || {};
  const classes = Array.isArray(printData?.classes) ? printData.classes : [];

  // ✅ LIVE school identity (hardcoded values kept as fallbacks)
  const schoolName = school.name || 'BimTech SaaS SolutionsS LIMITED(DISL)';
  const schoolAddress = school.address || '14, Ahmed Tijani St, Ahmadiya Bus-Stop, Lagos State, Nigeria.';
  const schoolLogoSrc = school.logoUrl || schoolLogo;
  const principalSignatureSrc = school.signatureUrl || principalSignature;
  ...
  return (
    <div className="print-view-wrapper">
      ...
      <div className="print-area">
        ...
        {classes.length === 0 ? (
          <div className="print-error-page">
            <p>No classes returned for this selection.</p>
            <button onClick={() => navigate(-1)}>Go Back</button>
          </div>
        ) : (
        classes.map((classObj, classIdx) => (
          <div
            key={docId(classObj?.classInfo) ?? `class-${classIdx}`}
            className="print-class-section"
            style={classIdx > 0 ? { pageBreakBefore: 'always' } : undefined}
          >
            {(Array.isArray(classObj?.students) ? classObj.students : []).map((student) => {
              // ✅ Safe field access throughout (Prisma shapes)
              ...
              const studentIdStr = docId(student.student)?.toString() || '';

              const principalComment = getPrincipalComment(docId(student.student));

              // Inject the photo from secondary fetch if missing on the print record
              ...
              const studentFullName = `${student.student?.firstName ?? ''} ${student.student?.lastName ?? ''}`.trim();

              return (
                <div key={studentIdStr || `student-${Math.random()}`} className="a4-document print-student-card">
                  {/* ===== SCHOOL HEADER (3-COLUMN: LOGO | INFO | PHOTO) ===== */}
                  <header className="school-header-elegant">
                    <div className="header-ornament top"></div>

                    <div className="header-content-row">
                      {/* LEFT - School Logo (✅ LIVE from site settings, bundled fallback) */}
                      <div className="header-logo-wrap">
                        <img
                          src={schoolLogoSrc}
                          alt={`${schoolName} Logo`}
                          className="header-logo"
                          onError={(e) => { if (e.currentTarget.src !== schoolLogo) e.currentTarget.src = schoolLogo; }}
                        />
                      </div>

                                           {/* CENTER - School Info (✅ LIVE from site settings) */}
                      <div className="header-school-info">
                        ...
                        {(school.website || school.email) && (
                          <div style={{ margin: '0.4mm 0 0 0', lineHeight: 1.45 }}>
                            {school.website && (
                              <p style={{ margin: 0, fontSize: '6.5pt', color: '#333', letterSpacing: '0.3px', textAlign: 'center' }}>
                                {school.website}
                              </p>
                            )}
                            {school.email && (
                  ...
                  <div className="grades-container">
                    <table className="grades-table-elegant">
                      ...
                      <tbody>
                        {subjects.map((sub, i) => (
                          <tr key={docId(sub) ?? sub.subjectId ?? i}>
                            <td className="td-center">{i + 1}</td>
                            {/* ✅ Tolerant subject name lookup */}
                            <td className="td-subject">{sub.subjectName || sub.subject?.name || sub.name || '—'}</td>
                  ...
                  <div className="comments-container">
                    ...
                    <div className="comment-box-elegant">
                      ...
                      <div className="signature-section" style={{ display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
                        {/* ✅ LIVE principal signature from site settings, bundled fallback */}
                        <img
                          src={principalSignatureSrc}
                          alt="Principal's Signature"
                          className="principal-sig-img"
                          onError={(e) => { if (e.currentTarget.src !== principalSignature) e.currentTarget.src = principalSignature; }}
                        />
                        <div className="sig-line"></div>
