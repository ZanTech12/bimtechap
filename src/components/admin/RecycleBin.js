...
// Helper to get ID safely whether it's from Mongoose (_id) or Prisma (id)
...
export default function RecycleBin() {
  ...
  const handleRestore = async (student) => {
    ...
    setRestoringIds((prev) => new Set(prev).add(studentId));
    try {
      await studentsAPI.restoreFromRecycleBin(studentId);
      setDeletedStudents((prev) => prev.filter((s) => getStudentId(s) !== studentId));
      setSelectedIds((prev) => {
  ...
  const handleBulkRestore = async () => {
    ...
    try {
      ...
      setDeletedStudents((prev) => prev.filter((s) => !selectedIds.has(getStudentId(s))));
  ...
  const toggleSelectAll = () => {
    if (selectedIds.size === filteredStudents.length) {
      ...
      setSelectedIds(new Set(filteredStudents.map((s) => getStudentId(s))));
    }
  };
          ...
          <table style={styles.table}>
            ...
            <tbody>
              {filteredStudents.map((student, idx) => {
                const studentId = getStudentId(student);
                const isRestoring = restoringIds.has(studentId);
                const isDeleting = deletingIds.has(studentId);
