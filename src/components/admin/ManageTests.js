...
// ✅ NEW: Normalize any ID shape (Mongoose populated object, Prisma raw Int,
// or already-plain value) into a string. Fixes "Invalid class or subject ID format."
const getId = (v) => {
...
const ManageTests = () => {
  ...
  // ✅ FIX: idStr() handles teacher_id as object, raw id, camelCase, etc.
  const myAssignments = useMemo(() => {
    if (!isTeacher || !assignmentsData?.data || !currentUserId) return [];
    const meId = String(currentUserId);
    return assignmentsData.data.filter((a) => idStr(a.teacher_id ?? a.teacherId) === meId);
  }, [isTeacher, assignmentsData, currentUserId]);

  const classSubjectMap = useMemo(() => {
    const map = {};
    myAssignments.forEach((a) => {
      // ✅ FIX: check raw class_id/classId too, not just populated objects
      const cId = idStr(a.class_id ?? a.classId);
      const sId = idStr(a.subject_id ?? a.subjectId);
  ...
  const allowedClassIds = useMemo(() => {
    ...
    return [...new Set(myAssignments.map(a => idStr(a.class_id ?? a.classId)).filter(Boolean))];
  }, [isTeacher, myAssignments]);

  const allowedSubjectIds = useMemo(() => {
    if (!isTeacher) return null;
    return [...new Set(myAssignments.map(a => idStr(a.subject_id ?? a.subjectId)).filter(Boolean))];
  }, [isTeacher, myAssignments]);

  const myClasses = useMemo(() => {
    if (!classes?.data) return [];
    if (!isTeacher || !allowedClassIds) return classes.data;
    // ✅ FIX: compare normalized strings
    return classes.data.filter(c => allowedClassIds.includes(idStr(c)));
  ...
  const mySubjectsForForm = useMemo(() => {
    ...
    if (!formData.classId) {
      ...
      return subjects.data.filter(s => allowedSubjectIds.includes(idStr(s)));
    }
    const allowed = classSubjectMap[idStr(formData.classId)] || [];
    return subjects.data.filter(s => allowed.includes(idStr(s)));
  ...
  const myTests = useMemo(() => {
    ...
    return tests.data.filter((test) => {
      const tC = idStr(test.classId);
      const tS = idStr(test.subjectId);
      return myAssignments.some(a => {
        const aC = idStr(a.class_id ?? a.classId);
        const aS = idStr(a.subject_id ?? a.subjectId);
        return aC === tC && aS === tS;
      });
    });
  ...
  const handleQuestionSetSelect = (e) => {
    ...
    if (qsId) {
      // ✅ FIX: normalized comparison (q._id number vs qsId string)
      const qs = questionSets?.data?.find(q => idStr(q) === qsId);
      if (qs) setFormData({ ...formData, questions: qs.questions, questionSetId: qsId });
    } else {
      setFormData({ ...formData, questions: [], questionSetId: '' });
    }
  };
  ...
  const handleOpenEdit = (test) => {
    if (isTeacher) {
      const tC = idStr(test.classId);
      const tS = idStr(test.subjectId);
      // ✅ FIX: normalized comparison on both sides
      const canEdit = myAssignments.some(a => idStr(a.class_id ?? a.classId) === tC && idStr(a.subject_id ?? a.subjectId) === tS);
      if (!canEdit) { setError('No permission to edit this test.'); return; }
    }
    setEditingTest(test);
    setFormData({
      title: test.title || '',
      // ✅ FIX: fall back to raw classId/subjectId for Prisma rows
      classId: idStr(test.classId), subjectId: idStr(test.subjectId),
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    if (isTeacher) {
      const ok = myAssignments.some(a => idStr(a.class_id ?? a.classId) === String(formData.classId) && idStr(a.subject_id ?? a.subjectId) === String(formData.subjectId));
      if (!ok) { setError('Not assigned to this class and subject.'); return; }
    }
    // ✅ formData.classId/subjectId are now guaranteed real IDs (strings of Ints)
    const data = { ...formData, duration: parseInt(formData.duration), passMark: parseInt(formData.passMark) };
    if (editingTest) updateMutation.mutate({ id: getId(editingTest), data });
  ...
  const handleDelete = (test) => {
    if (isTeacher) {
      const tC = idStr(test.classId);
      const tS = idStr(test.subjectId);
      if (!myAssignments.some(a => idStr(a.class_id ?? a.classId) === tC && idStr(a.subject_id ?? a.subjectId) === tS)) { alert('No permission to delete.'); return; }
    }
    if (window.confirm(`Delete "${test.title}"?`)) deleteMutation.mutate(getId(test));
  };
      ...
      {isTeacher && !hasNoAssignments && (
        <div className="mt-assign-bar">
          ...
          <div className="mt-assign-chips">
            {/* ✅ FIX: name fallbacks — shapeAssignments returns class_id/subject_id objects */}
            {myAssignments.map((a, i) => (
              <span key={getId(a) || i} className="mt-assign-chip">
                {a.class_name || a.class_id?.name || a.classId?.name || 'Class'} — {a.subject_name || a.subject_id?.name || a.subjectId?.name || 'Subject'}
              </span>
            ))}
          </div>
        </div>
            ...
            <table className="mt-table">
              ...
              <tbody>
                {myTests?.length === 0 ? (
                  ...
                  return (
                    <tr key={getId(test)}>
                      <td><div className="mt-td-title">{test.title}</div></td>
                      ...
                      <td>
                        <div className="mt-table-actions">
                          ...
                          {test.resultsPublished ? (
                            <button className="mt-btn mt-btn-warning-outline mt-btn-xs" onClick={() => handleUnpublish(getId(test))} title="Unpublish">
                            ...
                            <button className="mt-btn mt-btn-success-outline mt-btn-xs" onClick={() => handlePublish(getId(test))} title="Publish">
      ...
      {!hasNoAssignments && (
        <div className="mt-cards">
          {myTests?.length === 0 ? (
            ...
            const sm = getStatusMeta(test);
            return (
              <div className="mt-card" key={getId(test)}>
                ...
                <div className="mt-card-actions">
                  ...
                  {test.resultsPublished ? (
                    <button className="mt-btn mt-btn-warning-outline mt-btn-sm" onClick={() => handleUnpublish(getId(test))}>
                    ...
                    <button className="mt-btn mt-btn-success-outline mt-btn-sm" onClick={() => handlePublish(getId(test))}>
          ...
          <div className="mt-modal" onClick={(e) => e.stopPropagation()}>
            ...
            <div ref={modalBodyRef} onScroll={handleModalScroll} className="mt-modal-body">
              ...
              <form onSubmit={handleSubmit} id="mt-form">
                ...
                <div className="mt-form-row">
                  <div className="mt-form-group">
                    ...
                    <select name="classId" className="mt-form-select" value={formData.classId} onChange={handleChange} required>
                      ...
                      {/* ✅ FIX: idStr(c) — value is now a REAL ID, not the option text */}
                      {myClasses.map(c => { const cid = idStr(c); return <option key={cid} value={cid}>{c.name}{c.section ? ` — ${c.section}` : ''}</option>; })}
                    </select>
                  ...
                  <div className="mt-form-group">
                    ...
                    <select name="subjectId" className="mt-form-select" value={formData.subjectId} onChange={handleChange} required disabled={!formData.classId}>
                      ...
                      {/* ✅ FIX: idStr(s) — same normalization */}
                      {mySubjectsForForm.map(s => { const sid = idStr(s); return <option key={sid} value={sid}>{s.name}</option>; })}
                    </select>
                    {isTeacher && formData.classId && mySubjectsForForm.length === 0 && <span className="mt-form-hint mt-form-hint--err">No subjects assigned for this class.</span>}
                  </div>
                </div>

                {/* Question Set selector (bulk load) */}
                <div className="mt-form-group" style={{marginBottom:16}}>
                  <label className="mt-form-label">Question Set <span className="mt-form-hint">(optional — loads questions automatically)</span></label>
                  <select className="mt-form-select" value={selectedQuestionSet} onChange={handleQuestionSetSelect} disabled={!formData.classId || !formData.subjectId}>
                    <option value="">— Skip (no question set) —</option>
                    {(questionSets?.data || []).map(q => { const qid = idStr(q); return <option key={qid} value={qid}>{q.title} ({q.questions?.length || 0} questions)</option>; })}
                  </select>
                  {formData.classId && formData.subjectId && (questionSets?.data?.length === 0) && <span className="mt-form-hint">No question sets found for this class & subject.</span>}
                  {formData.questions.length > 0 && <span className="mt-form-qs-loaded">✓ {formData.questions.length} questions loaded</span>}
                </div>

                <div className="mt-form-row-3">
                  <div className="mt-form-group">
                    <label className="mt-form-label">Duration (minutes) *</label>
                ...
                <div className="mt-form-row">
                  <div className="mt-form-group">
                    <label className="mt-form-label">Start Date &amp; Time *</label>
                    <input type="datetime-local" name="startDate" className="mt-form-input" value={formData.startDate} onChange={handleChange} required />
                  </div>
                  <div className="mt-form-group">
                    <label className="mt-form-label">End Date &amp; Time *</label>
                    <input type="datetime-local" name="endDate" className="mt-form-input" value={formData.endDate} onChange={handleChange} required />
                  </div>
                </div>

                <div className="mt-form-group" style={{marginBottom:16}}>
                  <label className="mt-form-label">Instructions <span className="mt-form-hint">(shown to students)</span></label>
                  <textarea name="instructions" className="mt-form-textarea" value={formData.instructions} onChange={handleChange} placeholder="e.g., Answer all questions. No calculators allowed."></textarea>
                </div>
              </form>
            </div>
            ...
            <div className="mt-modal-footer">
              ...
              <button type="submit" form="mt-form" className="mt-btn mt-btn-primary" disabled={isSaving}>
                {isSaving ? (<><span className="mt-btn-spinner" /> Saving...</>) : (editingTest ? 'Update Test' : 'Create Test')}
              </button>
            </div>
          </div>
        </div>
